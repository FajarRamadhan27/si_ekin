...
import { assessmentShowYn, deleteEmployee, getAssessments, getAssessmentsHistory, getEmployeeRank } from '../../../utils/Axios';
...
export default function RankTable(props) {
  ...
  const [orderBy, setOrderBy] = React.useState('nilai_akhir');
  ...
  const { user } = useSelector(state => state)
    ...
    React.useEffect(() => {
        getEmployeeRank(setAssessment, value.format('YYYYMM'))
    }, [])
      ...
      <Paper sx={{ width: '100%', mb: 2 }}>
        ...
        <div className='flex mx-4 my-6 items-center justify-end text-sm '>
            <DatePickerCustom
                ...
                onChange={(newValue) => {
                    setValue(newValue)
                    getEmployeeRank(setAssessment, newValue.format('YYYYMM'))
                }}
